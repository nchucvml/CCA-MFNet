...
import networks
from dataset.datasets import GastricBinarySegDataSet
...
IMG_MEAN = np.array((22.00698793,14.66876762,10.67891434), dtype=np.float32) # For Gastric IM

BATCH_SIZE = 1
DATA_DIRECTORY = 'gastric_dataset'

### 224x224
# DATA_LIST_PATH = './dataset/list/gastric_dataset/train.lst'
DATA_LIST_PATH = ''
...
MOMENTUM = 0.7
NUM_CLASSES = 2
NUM_STEPS = 541100 # Iteration number
POWER = 0.9
RANDOM_SEED = 12345
RESTORE_FROM = ''
SAVE_NUM_IMAGES = 2
SAVE_PRED_EVERY = 38650
...
def main():
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        dataset = GastricBinarySegDataSet(args.data_dir, args.data_list, max_iters=None, crop_size=input_size,
                    scale=args.random_scale, mirror=args.random_mirror, mean=IMG_MEAN) # 20220630
        ...
        while run:
            ...
            for idx in pbar:
                ...
                images, labels, _1, _2 = dataloader.next()
                images = images.cuda()
                ...
                if (not engine.distributed) or (engine.distributed and engine.local_rank == 0):
                    if global_iteration % args.save_pred_every == 0 or global_iteration >= args.num_steps:
                        ...
                        torch.save(seg_model.state_dict(),osp.join(args.snapshot_dir, 'CCA_MFNet_Model_Iter_'+str(global_iteration)+'.pth'))
